...
const executorType = arg('executor-type') ?? 'official_api';
if (executorType !== 'official_api' && executorType !== 'browser') throw new Error('--executor-type must be official_api or browser');
...
const route = executorType === 'official_api'
  ? { executorType: 'official_api' as const, platform: application.targetPlatform, accountRef, credentialRef: `${application.targetPlatform}-credential:runtime` }
  : { executorType: 'browser' as const, platform: application.targetPlatform, accountRef, sessionRef: `${application.targetPlatform}-session:runtime` };
...
process.stdout.write(`${JSON.stringify({ status: 'APPROVAL_CREATED', approvalId, actionId: approval.actionId, payloadHash: approval.payloadHash, executorType, expiresAt, output }, null, 2)}\n`);
